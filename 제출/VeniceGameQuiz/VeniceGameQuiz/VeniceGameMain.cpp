...
o 7. 아이템기능 추가
	...
	o > 아이템 적용 여부 랜덤 결정(5%)
8. 추가 할 일
	o > 맵 관리 기능 클래스 분리하기(배경 관리).
		o >> 인터페이스 클래스 만들기(정지된 배경관리)
			o -> 배경 그리기 :(로비화면), (스토리 스킵화면),(이름 입력 화면), (게임 화면), (스테이지 업), (게임오버 화면), (랭킹 화면) 등등....
	o > 단어 맞추기 관련 함수 세분화(역할별로 나눌 것)
